...
async function main() {
  ...
  try {
    ...
    logger.info(
      `✅ Step 0 Complete: Task fetched in ${((Date.now() - step0Start) / 1000).toFixed(2)}s`,
    );
    ...
    logger.info(
      `✅ Step 1 Complete: Script generated in ${((Date.now() - step1Start) / 1000).toFixed(2)}s`,
    );
    ...
    const GEN_AUDIO = path.join(
      __dirname,
      "../audio",
      `cloned_voice_${Date.now()}.wav`,
    );
    ...
    if (!fs.existsSync(REF_AUDIO)) {
      throw new Error(
        `CRITICAL: Base-audio.mp3 not found in assets. Voicebox requires this reference file.`,
      );
    }

    // Single-pass synthesis with professional technical educator instructions
    const VOICE_INSTRUCT =
      "Steady, authoritative technical educational delivery. Professional and clear. Speak naturally with brief pauses at punctuation.";
    const rawAudioPath = await voiceboxService.generateClonedVoice(
      script,
      REF_AUDIO,
      GEN_AUDIO,
      null,
    ...
    const slowedAudio = path.join(
      path.dirname(GEN_AUDIO),
      `slowed_${path.basename(GEN_AUDIO)}`,
    );
    ...
    /* GEMINI TTS FALLBACK (Commented out)
        ...
        */
    logger.info(
      `✅ Step 2 Complete: Audio ready at ${audioPath} (${((Date.now() - step2Start) / 1000).toFixed(2)}s)`,
    );
    ...
    logger.info(
      `✅ Step 3 Complete: Wav2Lip sync success in ${((Date.now() - step3Start) / 1000).toFixed(2)}s`,
    );
    ...
    logger.info(
      `✅ Step 4 Complete: SRT generated in ${((Date.now() - step4Start) / 1000).toFixed(2)}s`,
    );
    ...
    if (!visualPrompt || visualPrompt.length < 10) {
      throw new Error(
        "CRITICAL: Visual prompt generation returned no usable content. Aborting to avoid empty video.",
      );
    }

    fs.writeFileSync("visual_prompt.txt", visualPrompt);
    logger.info(
      `✅ Step 5 Complete: Visual prompt ready in ${((Date.now() - step5Start) / 1000).toFixed(2)}s`,
    );

    // Step 6: Playwright Logic
    currentStep = "Browser Recording & Composition";
    logger.info(
      "🌐 Step 6: Recording Unified Visuals via Browser Compositor...",
    );
    const step6Start = Date.now();
    const finalMasterPath = await runCompositor(
      INIT_MERGE,
      srtPath,
      visualPrompt,
    );
    if (!finalMasterPath)
      throw new Error("Browser recording failed to produce a file.");
    logger.info(
      `✅ Step 6 Complete: MASTER REEL created at ${finalMasterPath} (${((Date.now() - step6Start) / 1000).toFixed(2)}s)`,
    );

    // Step 6.5: Prepend static cover image for first 1 second (for Instagram cover consistency)
    currentStep = "Applying Cover Intro";
    logger.info(
      "🖼️ Step 6.5: Prepending cover intro frame (assets/cover.png)...",
    );
    const step65Start = Date.now();
    const videoForPosting = await prependCoverIntro(finalMasterPath);
    logger.info(
      `✅ Step 6.5 Complete: Cover intro applied in ${((Date.now() - step65Start) / 1000).toFixed(2)}s`,
    );
    ...
    logger.info(
      `✅ Step 7 Complete: Captions ready in ${((Date.now() - step7Start) / 1000).toFixed(2)}s`,
    );
    ...
    try {
      ...
      supabaseInfo = await uploadToSupabaseAndGetLink(videoForPosting, TOPIC);
    } catch (e) {
      logger.error(`❌ Supabase Upload Error: ${e.message}`);
    }

    // A. YouTube Upload (Direct - Independent of Staging)
    const ytPromise = uploadToYouTube(videoForPosting, TOPIC, social.caption)
      .then((res) => {
        if (res.success) {
          links.yt = res.url;
          logger.info(`✅ YT Posted: ${res.url}`);
        }
      })
    ...
    if (supabaseInfo && supabaseInfo.success) {
      ...
      metaPromises.push(
        uploadToInstagramWithUrl(publicUrl, TOPIC, social.caption)
          .then((res) => {
            if (res.success) {
              links.insta = res.url;
              logger.info(`✅ Insta Posted: ${res.url}`);
            }
          })
      ...
      logger.warn(
        "⚠️ Meta (Instagram/Facebook) upload skipped because Supabase staging failed.",
      );
    }
    ...
    logger.info(
      `✅ Step 8 Complete: ${successCount}/3 uploads finished in ${((Date.now() - step8Start) / 1000).toFixed(2)}s`,
    );
    ...
    if (successCount > 0) {
      ...
      await sendSuccessNotification(task, links).catch((e) =>
        logger.error(`❌ Email failed: ${e.message}`),
      );
    } else {
      // If ALL 3 failed, we treat it as a total failure
      const error = new Error(
        "All social media uploads failed (YouTube, Instagram, and Facebook). Check logs for details.",
      );
      throw error;
    }
    ...
    logger.error(
      `❌ PIPELINE FAILED | Step: [${currentStep}] | Error: ${err.message}`,
    );
    ...
    await sendErrorNotification(task, err, currentStep).catch((e) =>
      logger.error(`❌ Error email failed: ${e.message}`),
    );
  }
}
...
async function prependCoverIntro(inputVideoPath) {
  ...
  if (!fs.existsSync(coverPath)) {
    throw new Error(
      `CRITICAL: Cover image not found at ${coverPath}. Expected assets/cover.png`,
    );
  }
  ...
  if (!videoStream) {
    throw new Error(`No video stream found in ${inputVideoPath}`);
  }

  const hasAudio = probeInfo.streams.some((s) => s.codec_type === "audio");
  const width = videoStream.width || 1080;
  const height = videoStream.height || 1920;
  const outputPath = path.resolve(`FINAL_REEL_COVERED_${Date.now()}.mp4`);

  const filterGraph = [
    `[0:v]scale=${width}:${height}:force_original_aspect_ratio=decrease,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2,setsar=1,format=yuv420p[cover]`,
    `[1:v]setpts=PTS-STARTPTS[mainv]`,
    `[cover][mainv]concat=n=2:v=1:a=0[vout]`,
  ];

  if (hasAudio) {
    filterGraph.push(
      `[1:a]adelay=${delayMs}|${delayMs},asetpts=PTS-STARTPTS[aout]`,
    );
  }
  ...
  await new Promise((resolve, reject) => {
    let command = ffmpeg()
      .input(coverPath)
      .inputOptions(["-loop 1", `-t ${coverDurationSec}`])
    ...
    if (hasAudio) {
      command = command.outputOptions(["-c:a aac", "-ar 44100", "-map [aout]"]);
    }
  ...
  if (!fs.existsSync(outputPath)) {
    throw new Error(
      `Cover-intro render completed but output was not found: ${outputPath}`,
    );
  }
...
async function runCompositor(vPath, sPath, vPrompt) {
  ...
  page.on("download", async (dl) => {
    ...
    console.log(
      "📽️ Final Remuxing (audio offset sync + BGM mixing + high-compatibility settings)...",
    );
  ...
  try {
    ...
    if (await pass.isVisible()) {
      ...
      for (const key of uniqueKeys) {
        console.log(
          `🔑 Trying API key: ${key.substring(0, 4)}...${key.substring(key.length - 4)}`,
        );
        await pass.fill(key);
    ...
    await page
      .waitForSelector("#video-upload", { state: "attached", timeout: 60000 })
      .catch((err) => {
        throw new Error(
          `CRITICAL: Page failed to load #video-upload after login attempts. Error: ${err.message}`,
        );
      });
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      const status = await page.evaluate(() => {
        ...
        const isGenerating =
          studioBtn?.disabled ||
          document.body.innerText.includes("Generating Scene...");
    ...
    if (!isDone) {
      throw new Error(
        "CRITICAL: Visual generation timed out or failed to render in browser (10 minutes). Aborting to avoid black video.",
      );
    }

    console.log("⏳ Looking for Rec & Export button...");
    await page.waitForSelector('button:has-text("Rec & Export")', {
      timeout: 30000,
    });
    ...
    if (finalCheck !== "OK") {
      console.warn(
        `⚠️ Warning: Video state is ${finalCheck}. Attempting to force reload src...`,
      );
      await page.evaluate(() => {
        const v = document.querySelector("video");
        if (v) {
    ...
    const isRecordingStarted = await page
      .waitForSelector('button:has-text("Stop Recording")', { timeout: 15000 })
      .then(() => true)
      .catch(() => false);
    if (!isRecordingStarted) {
      console.warn(
        "⚠️ Recording failed to start automatically. Trying to force play...",
      );
      await page.evaluate(() => {
        const v = document.querySelector("video");
        if (v) v.play();
    ...
    while (!complete && Date.now() - begin < 900000) {
      // 15 mins timeout
      await page.waitForTimeout(5000);
      ...
      if (Date.now() - lastReport > 15000) {
        console.log(
          `📹 Recording Progress: ${stats.time.toFixed(1)}s / ${stats.duration.toFixed(1)}s (Paused: ${stats.paused}, Ended: ${stats.ended})`,
        );
        lastReport = Date.now();
      }
      ...
      if (
        stats.ended ||
        (stats.duration > 0 && stats.time >= stats.duration - 0.5)
      ) {
        console.log(
          "🎞️ Playback reached end. Stopping recording and waiting for download...",
        );
        ...
        const wait = Date.now();
        while (!complete && Date.now() - wait < 300000)
    ...
    if (!complete) {
      console.error(
        "❌ Recording/Download timed out! Attempting emergency screenshot.",
      );
    }
  } catch (e) {
