...
const getNextTask = async () => {
  try {
    ...
    // Find column indexes
    const snoIndex = headers.findIndex(h => h.toLowerCase().includes('sno') || h.toLowerCase().includes('s.no') || h.toLowerCase().includes('serial'));
    const ideaIndex = headers.findIndex(h => h.toLowerCase().includes('idea') || h.toLowerCase().includes('title'));
    const descriptionIndex = headers.findIndex(h => h.toLowerCase().includes('description'));
    const statusIndex = headers.findIndex(h => h.toLowerCase().includes('status'));
    const ytLinkIndex = headers.findIndex(h => h.toLowerCase().includes('yt') || h.toLowerCase().includes('youtube'));
    const instaLinkIndex = headers.findIndex(h => h.toLowerCase().includes('insta') || h.toLowerCase().includes('instagram'));
    const timestampIndex = headers.findIndex(h => h.toLowerCase().includes('timestamp') || h.toLowerCase().includes('date'));

    logger.info(`Found column indexes: SNO=${snoIndex}, Idea=${ideaIndex}, Description=${descriptionIndex}, Status=${statusIndex}`);
 ...
 */
const updateSheetStatus = async (rowId, status, ytLink = '', instaLink = '') => {
  try {
    const sheets = await getSheetsClient();
